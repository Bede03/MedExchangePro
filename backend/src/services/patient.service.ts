      ...
      try {
        ...
        return {
          ...
          medications: prescriptions.map((rx: any) => ({
            ...
            prescribedBy: rx.PRESCRIBER_NAME || (rx.PRESCRIBED_BY ? `Staff ID ${rx.PRESCRIBED_BY}` : 'Unknown'),
            prescriberName: rx.PRESCRIBER_NAME || 'Unknown',
            medicationName: [rx.BRAND_NAME, rx.GENERIC_NAME].filter(Boolean).join(' - ') || 'Medication',
